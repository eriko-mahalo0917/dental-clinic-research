...
### popup.py
処理完了後に結果を知らせるポップアップを表示します。

- 処理件数を表示（0件の場合も通知）
- Mac・Windows どちらでも動作
- Mac では OS レベルで最前面に表示（pyobjc 使用）
- Windows では PySide6 の標準機能で最前面に表示

### path_helper.py
実行ファイル（.py）と exe ファイル（PyInstaller でビルドした場合）の両方に対応したパス取得を担当します。

- `creds.json` のパスを返す
- `.env` のパスを返す
- exe として実行中か、Python として実行中かを自動で判定
...
> **Mac・Windows 両対応について**  
> `requirements.txt` には Mac 専用のライブラリ（`pyobjc-framework-Cocoa` など）が含まれていますが、  
> `; sys_platform == "darwin"` という条件付きで記載されているため、  
> Windows では自動的にスキップされます。  
> Mac・Windows どちらでも同じコマンドでインストールできます。
...
├ main.py
├ requirements.txt
├ .env            ※ APIキーを記載（Gitには含めない）
├ creds.json      ※ Google認証ファイル（Gitには含めない）
└ README.md
```
